...
class ValuWidget
{
        ...
        if ($valu_payment->valu_widget_enable) {
            ...
            if ($product_price) {
                if ($product_price >= $valu_payment->valu_widget_price_threshold) {
                    ...
                    if ($plan) {
                        include(CLICKPAY_PAYPAGE_DIR . 'includes/_valu_widget.php');
                    }
                }
            }
        }
    ...
    {
        $_clickpayApi = ClickpayApi::getInstance($valu_payment->clickpay_endpoint, $valu_payment->merchant_id, $valu_payment->merchant_key);
        ...
        ClickpayHelper::log("valU inqiry, {$product_price}", 1);
        $details = $_clickpayApi->inqiry_valu($data);

        if (!$details || !$details->success) {
            $_err_msg = json_encode($details);
            ClickpayHelper::log("valU Details error: [{$_err_msg}]", 3);
            return false;
        }
        ...
        try {
            ...
            ClickpayHelper::log("valU widget error: " . $th->getMessage(), 3);
        }
    ...
    private function getValUPlan($details, $installments_count)
    {
        try {
            ...
            ClickpayHelper::log("valU Plan error: " . $th->getMessage(), 3);
        }

        $_log = json_encode($plansList);
        ClickpayHelper::log("valU Plan error: No Plan selected, [{$_log}]", 2);
