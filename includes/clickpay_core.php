...
define('CLICKPAY_SDK_VERSION', '2.21.0');
...
abstract class ClickpayHelper
{
    ...
    static function supportPending($code)
    {
        foreach (ClickpayApi::PAYMENT_TYPES as $key => $value) {
            if ($value['name'] === $code) {
                return in_array(ClickpayApi::GROUP_PENDING, $value['groups']);
            }
        }
        return false;
    }
    ...
    {
        return preg_match(ClickpayEnum::DISCOUNT_PATTERN_REGEX, $pattern);
    }
...
abstract class ClickpayEnum
{
    ...
    {
        return strcasecmp($tran_type, ClickpayEnum::TRAN_TYPE_AUTH_EXT) == 0;
    }
 ...
 *  -- pnr_code
 */
abstract class ClickpayExtraDataHolder extends ClickpayHolder
{
 ...
 * - User defined
 */
abstract class ClickpayBasicHolder extends ClickpayExtraDataHolder
{
    /**
     * payment_type
     */
     ...
     * @param bool $optional Display the save card option on the payment page
     */
    public function set11TokeniseInfo($on, $token_format = 2, $token_type = ClickpayEnum::TOKEN_TYPE_UNSCHEDULED, $counter = null, $total_count = null, $optional = false)
    {
        if ($on) {
            $_info = [
                'tokenise' => $token_format,
...
class ClickpayRequestHolder extends ClickpayBasicHolder
{
    ...
    public function set13CardDiscounts($discount_patterns, $discount_amounts, $discount_types)
    {
        if (empty($discount_patterns)) {
            ClickpayHelper::log('Clickpay admin: Discount arrays must be not empty', 3);
            return $this;
        }
        ...
        if ($count != count($discount_amounts) || $count != count($discount_types)) {
            ClickpayHelper::log('Clickpay admin: Discount arrays must have the same length', 3);
            return $this;
        }
 ...
 */
class ClickpayTokenHolder extends ClickpayExtraDataHolder
{
    /**
     * token
     * tran_ref
...
class ClickpayApi
{
    ...
    const PAYMENT_TYPES = [
        '0'  => ['name' => 'all', 'title' => 'Clickpay - All', 'currencies' => null, 'groups' => [ClickpayApi::GROUP_TOKENIZE, ClickpayApi::GROUP_AUTH_CAPTURE, ClickpayApi::GROUP_IFRAME, ClickpayApi::GROUP_REFUND, ClickpayApi::GROUP_PENDING]],
        '1'  => ['name' => 'applepay', 'title' => 'Clickpay - ApplePay', 'currencies' => null, 'groups' => [ClickpayApi::GROUP_TOKENIZE, ClickpayApi::GROUP_AUTH_CAPTURE, ClickpayApi::GROUP_REFUND]],
        '2'  => ['name' => 'mada', 'title' => 'Clickpay - mada', 'currencies' => ['SAR'], 'groups' => [ClickpayApi::GROUP_TOKENIZE, ClickpayApi::GROUP_CARDS, ClickpayApi::GROUP_AUTH_CAPTURE, ClickpayApi::GROUP_IFRAME, ClickpayApi::GROUP_REFUND]],
        '3'  => ['name' => 'creditcard', 'title' => 'Clickpay - CreditCard', 'currencies' => null, 'groups' => [ClickpayApi::GROUP_TOKENIZE, ClickpayApi::GROUP_CARDS, ClickpayApi::GROUP_CARDS_INTERNATIONAL, ClickpayApi::GROUP_AUTH_CAPTURE, ClickpayApi::GROUP_IFRAME, ClickpayApi::GROUP_REFUND]],
        '4' => ['name' => 'amex', 'title' => 'Clickpay - Amex', 'currencies' => ['AED', 'SAR', 'USD'], 'groups' => [ClickpayApi::GROUP_TOKENIZE, ClickpayApi::GROUP_CARDS, ClickpayApi::GROUP_CARDS_INTERNATIONAL, ClickpayApi::GROUP_AUTH_CAPTURE, ClickpayApi::GROUP_IFRAME, ClickpayApi::GROUP_REFUND]],
        '5' => ['name' => 'tabby', 'title' => 'Clickpay - Tabby', 'currencies' => ['AED', 'SAR'], 'groups' => []],
    ];
    ...
    private function __construct($region, $profile_id, $server_key)
    {
        if (array_key_exists($region, self::BASE_URLS)) {
            ...
            ClickpayHelper::log("Clickpay Admin: Region {$region} is not valid", 2);
        }
