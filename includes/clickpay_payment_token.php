...
function get_account_saved_payment_methods_list_item_clickpay($item, $payment_token)
{
    if ('clickpay' === strtolower($payment_token->get_type())) {
        $item['method']['last4'] = $payment_token->get_last4();
        // $item['method']['expire'] = $payment_token->get_expiry_year();
        $item['method']['brand'] = isset($item['method']['gateway']) ? $item['method']['gateway'] : ('Clickpay, ' . $payment_token->get_display_name());
